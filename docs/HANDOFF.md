...
**일시 중지**: 2026-05-02
**진행 상태**: **v0.6.2 release** — 무공 돌파속성 (extraAddData / speEquipData / speUseData / etc.) 풀 복원 + Stat snapshot/restore (Stat unchecked 시 부수효과 보호). 13-카테고리 시리즈 polish.
...
**현재 main baseline = v0.6.2** (selection-aware Apply / Restore + 13-카테고리 체크박스 UI + 무공 돌파속성 풀 복원 + Stat snapshot/restore).
