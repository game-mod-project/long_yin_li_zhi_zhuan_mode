...
**일시 중지**: 2026-05-02
**진행 상태**: **v0.6.3 release** — 2D nested list (treasure.playerGuessTreasureLv 등 List<List<int>>) 풀 복원. Activator.CreateInstance 로 inner list 인스턴스 생성 후 recurse.
...
- **partPosture sub-data**: 슬롯 JSON 캡처 안 됨 — 별도 외형 sub-feature 필요시 후속.
...
**현재 main baseline = v0.6.3** (selection-aware Apply / Restore + 13-카테고리 체크박스 UI + 무공 돌파속성 풀 복원 + Stat snapshot/restore + 2D nested list 풀 복원).
