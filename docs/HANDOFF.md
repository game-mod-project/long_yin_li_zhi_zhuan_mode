...
**일시 중지**: 2026-05-09
**진행 상태**: **v0.7.10 release** — 천부 max 보유수 lock (Phase 1) + 속성·무학·기예 editor (Phase 2). PlayerEditorPanel `[기본 / 속성]` secondary tab — 속성 6 / 무학 9 / 기예 9 (24 row × 2 axis 수치/자질값) inline TextField + 일괄 button × 3 + [저장] gated apply (cheat ChangeAttri/FightSkill/LivingSkill mirror + RefreshMaxAttriAndSkill 1회). 천부 섹션 헤더 [Lock max] 토글 (HeroData.GetMaxTagNum Postfix override, Player heroID=0 only, ConfigEntry 영속). 367 tests PASS + 인게임 smoke 18/18 PASS (smoke run 후 갱신).
