...
**일시 중지**: 2026-05-03
**진행 상태**: **v0.7.2 release** — 컨테이너 검색·정렬 (글로벌 toolbar + 3-area cache + 등급 6단계 색상). 기존 v0.7.1 capacity UI 위에 D-3 sub-project 가 추가됨. 133/133 tests PASS.
...
**현재 main baseline = v0.7.2** (컨테이너 검색·정렬 — 글로벌 toolbar + 3-area cache + itemLv/rareLv reflection + 등급 6단계 색상. 133/133 tests PASS).
...
1. ✅ **v0.7.2 D-3 검색·정렬** (release 완료 — 본 commit)
...
3. **v0.7.4 D-1 Item 상세 panel** — 선택 item 의 reflection 데이터 (속성·강화·내구도 등) 표시. v0.7.2 ItemReflector 확장 + sub-data wrapper 별 reflection 추가.
...
> LongYin Roster Mod — **main baseline = v0.7.2** (2026-05-03 release). D 계열 첫 sub-project (D-3 검색·정렬) 완료.
> 프로젝트 루트: `E:/Games/龙胤立志传.v1.0.0f8.2/LongYinLiZhiZhuan/Save/_PlayerExport/`.
> 핸드오프: `docs/HANDOFF.md`. v0.7.2 spec: `docs/superpowers/specs/2026-05-03-longyin-roster-mod-v0.7.2-design.md`. plan: `docs/superpowers/plans/2026-05-03-longyin-roster-mod-v0.7.2-plan.md`.
> v0.7.2 spike: `docs/superpowers/dumps/2026-05-03-v0.7.2-grade-quality-spike.md`.
>
> **v0.7.2 결과**: ContainerPanel 글로벌 toolbar (검색 box 200 + 카테고리/이름/등급/품질 70/60/60/60 + ▲/▼ 32 — 총 490px). 인벤·창고·컨테이너 3-area 가 같은 SearchSortState 공유 + 각자 ContainerView (cache). 등급 6단계 row text 색상 (열악 #9CA3AF 회색 → 절세 #EF4444 빨강).
>
> **v0.7.2 spike 발견** (다음 sub-project 자산):
> - `itemLv` (Int32, 0..5) = 등급 (劣→普→优→精→完美→绝世)
> - `rareLv` (Int32, 0..5) = 품질 (残→下→中→上→珍→极)
> - ItemData sub-data wrapper (`equipmentData`/`medFoodData`/`bookData`/`treasureData`/`materialData`/`horseData`) presence — v0.7.4 D-1 sub-data reflection 자산
> - 사용자가 한글 패치 mod 사용 중 — "절세大劍" 같이 한글 grade prefix + 한자 이름 합성 표시. v0.7.5 D-4 한글화 시 사전 hook 가능성 조사 필요
>
> **다음 단계 후보**:
> - **v0.7.3 D-2 아이콘 그리드** — sprite reference + IMGUI grid (IL2CPP sprite asset 접근 challenge)
> - **v0.7.4 D-1 Item 상세 panel** — 선택 item reflection 표시 (sub-data wrapper 별 추가 reflection)
