...
**일시 중지**: 2026-05-03
**진행 상태**: **v0.7.1 release** — 컨테이너 UX 1차: destination 명시 (대칭 mirror 4-callback), 무게 표시 (`(N개, X.X / MAX kg)`), 가드 (인벤 over-weight 허용+속도페널티 안내, 창고 hard cap+거절 toast). Capacity reflection (`ItemListData.maxWeight` Single) + Config fallback. 111/111 tests PASS.
...
**현재 main baseline = v0.7.1** (컨테이너 UX 1차 — destination 명시 + 무게 표시 + destination 별 가드. spike 결과 갯수가 아닌 **무게 (kg, float)** 기반 확정. 111/111 tests PASS).

**다음 세션 후속 sub-project** (v0.7.1 smoke 도중 추가 발견 1건 통합 — D-4 Item 한글화 신규):
- v0.7.2: D-1 컨테이너 UX 2차 — Item 상세 panel (선택 item reflection 표시)
- v0.7.3: D-2 컨테이너 UX 3차 — 아이콘 그리드 (sprite reference + IMGUI grid)
- v0.7.4: D-3 컨테이너 UX 4차 — 검색·정렬 (이름·카테고리·강화 등)
- v0.7.5: D-4 Item 한글화 (smoke 발견) — 한글패치 사전 hook 또는 자체 itemID→한글 사전. ContainerPanel item 이름이 현재 중국어 한자 노출
...
## 6. 다음 세션 — v0.7.2+ sub-project 또는 maintenance 모드

v0.7.1 출시 완료 (2026-05-03). 컨테이너 UX 1차 (destination 명시 + 무게 표시 + destination 별 가드) 인게임 smoke 6/6 PASS.
...
**컨테이너 D 계열** (v0.7.1 mirror layout 위에 점진적 추가):
1. **v0.7.2 D-1 Item 상세 panel** — 선택 item 의 reflection 데이터를 우측 또는 별도 패널에 표시 (속성·강화·내구도 등)
2. **v0.7.3 D-2 아이콘 그리드** — sprite reference (item.iconID) IMGUI grid 표시. challenge: IL2CPP sprite asset 접근, IMGUI texture caching
3. **v0.7.4 D-3 검색·정렬** — 이름·카테고리·강화 등 query, 클라이언트 측 filter 만 (별도 데이터 없음)
4. **v0.7.5 D-4 Item 한글화** — 한글 패치 mod 사전 hook 또는 자체 itemID→한글 사전. D-1/2/3 작업 시점에 통합 가치 큼 (Item 표시 영역 동시 손볼 때)

**캐릭터 관리 계열** (D 끝나면):
5. v0.7.6 설정 panel — hotkey / 컨테이너 정원 / 창 크기
6. v0.7.7 Apply 부분 미리보기
7. v0.7.8 Slot diff preview
8. v0.7.9 NPC 지원

**maintenance 모드** — 게임 patch (v1.0.0 f8.3+) 시 재검증 + breakage fix.
...
> LongYin Roster Mod — **main baseline = v0.7.1** (2026-05-03 release). 컨테이너 UX 1차 완료.
> 프로젝트 루트: `E:/Games/龙胤立志传.v1.0.0f8.2/LongYinLiZhiZhuan/Save/_PlayerExport/`.
> 핸드오프: `docs/HANDOFF.md`. v0.7.1 spec: `docs/superpowers/specs/2026-05-03-longyin-roster-mod-v0.7.1-design.md`. plan: `docs/superpowers/plans/2026-05-03-longyin-roster-mod-v0.7.1-plan.md` (amendment 참조).
> v0.7.1 dumps: `docs/superpowers/dumps/2026-05-03-v0.7.1-{container-ux-backlog,capacity-spike,smoke-results}.md`.
>
> **v0.7.1 결과**: ContainerPanel destination 명시 (대칭 mirror 4-callback) + 무게 표시 (`(N개, X.X / MAX kg)`) + 가드 (인벤 over-weight 허용+속도페널티 안내, 창고 hard cap+거절 toast). spike 로 갯수가 아닌 **무게 (Single/float, kg)** 기반 확정. 111/111 tests PASS.
>
> **v0.7.1 결정적 발견** (다음 sub-project 자산):
> - `ItemListData.maxWeight` (Single) — capacity property. ItemListData 에 갯수 기반 capacity 자체 부재
> - `ItemListData.weight` (Single) — 현재 무게 (root-level field 가 SelfStorageApplier 주석에서 명시). reflection 우선 + wrapper.weight 합산 fallback 패턴
> - `ItemRow.Weight` 가 ContainerRowBuilder 에서 이미 reflection 으로 채워짐
> - `Core.ItemListReflector.GetMaxWeight(obj, fallback)` helper — 다른 sub-data property 도 같은 패턴 적용 가능
> - 사용자 한글 패치 mod 사용 중 — 본 모드 ContainerPanel item 이름이 한자 그대로 (D-4 한글화 sub-project 등록)
>
> **다음 단계 후보**:
> - **v0.7.2 D-1 Item 상세 panel** — 선택 item reflection 표시. ContainerPanel 우측 또는 별도 panel
> - **v0.7.3 D-2 아이콘 그리드** — sprite reference + IMGUI grid (IL2CPP sprite/texture cache challenge)
> - **v0.7.4 D-3 검색·정렬** — 클라이언트 측 filter
> - **v0.7.5 D-4 Item 한글화** — 한글 패치 사전 hook 또는 자체 사전. D-1/2/3 와 통합 가치 큼
