...
| # | 질문 | PoC 단계 |
|---|---|---|
| Q1 | 정체성 9 필드의 일관 우회 path — setter / backing field / Harmony 중 어느 것이 풀림? 일부만 풀리면 per-field 매핑? | Plan Task 2 |
| Q2 | nowActiveSkill 의 SetNowActiveSkill 가 wrapper reference 받을 때 — kungfuSkills 안의 같은 skillID 가진 wrapper 가 항상 1개? 중복 / 빈 list 처리? | Plan Task 3 |
| Q3 | ItemData wrapper ctor / factory — IntPtr 직접 / static factory / Harmony hijack 중 어느 것이 풀림? | Plan Task 4 |
| Q4 | LoseAllItem() 의 부수효과 — 장비 자동 해제? 퀘스트 아이템 보호? | Plan Task 4 smoke |
| Q5 | selfStorage.allItem 의 clear 방법 — IL2CppListOps.Clear raw 가 안전한가? game-self method 없음 | Plan Task 4 |
